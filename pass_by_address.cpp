...
class Account
{
    ...
    public:
            ...
            void transfer_address(Account *obj1,Account *obj2,double ammount_transfer)
            {
                if((*obj1).balance>2000.0)
                {
                    (*obj1).balance-=ammount_transfer;
                    (*obj2).balance+=ammount_transfer;
                    cout<<"The balance in transfer_address for obj1:"<<(*obj1).balance<<endl;
                    cout<<"The balance in transfer_address obj2:"<<(*obj2).balance<<endl;
                }
                else
...
int main()
{
    ...
    a1.display();
    a2.display();
    a1.transfer_address(&a1,&a2,500.0);
    a1.display();
    a2.display();
    return 0;
}
